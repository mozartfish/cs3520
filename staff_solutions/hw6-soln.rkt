...
(module+ test
  ...
  (test (interp (parse `{pair 1 2}) mt-env)
        (pairV (delay (numE 1) mt-env (box (none)))
               (delay (numE 2) mt-env (box (none)))))
...
(define (interp-expr [a : Exp]) : S-Exp
  (type-case Value (interp a mt-env)
    ...
    ;; Use `else` to cover `pairV` and impossible `suspendV` case
    [else `pair]))
...
(module+ test
  ...
  ;; Too slow if `fst` is unsufficiently lazy
  (test (interp-expr 
         (parse 
          `{let {[mkrec
                  {lambda {body-proc}
  ...
  ;; Too slow if `rst` is insufficiently lazy
  (test (interp-expr 
         (parse 
          `{let {[mkrec
                  {lambda {body-proc}
