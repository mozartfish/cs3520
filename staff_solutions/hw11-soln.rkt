...
(module+ test
  ...
  (test (interp (parse `{if0 1 2 3})
                mt-env)
        (numV 3))
  (test/exn (interp (parse `{if0 {cons 1 empty} 2 3})
                    mt-env)
            "not a number")
...
(define (typecheck [a : Exp] [tenv : Type-Env])
  (type-case Exp a
    ...
    [(consE l r)
     (let ([result-type (listofT (typecheck l tenv))])
       (begin
         (unify! result-type
                 (typecheck r tenv)
                 r)
    ...
    [(firstE a)
     (let ([result-type (varT (box (none)))])
       (begin
         ...
         result-type))]
    [(restE a)
     (let ([result-type (listofT (varT (box (none))))])
       (begin
         (unify! result-type
                 (typecheck a tenv)
                 a)
    ...
    [(if0E tst thn els)
     (begin
       ...
       (let ([result-type (typecheck thn tenv)])
         (begin
           (unify! result-type (typecheck els tenv) els)
           result-type)))]))
...
(module+ test
  ...
  (test (resolve (typecheck (parse `{cons 1 empty})
                            mt-env))
        (listofT (numT)))
  (test (resolve (typecheck (parse `{+ 1 {first empty}})
                            mt-env))
        (numT))
  (test (resolve (typecheck (parse `{cons 1 {rest empty}})
                            mt-env))
        (listofT (numT)))
  
  (test (resolve (typecheck (parse `{if0 1 2 3})
                            mt-env))
        (numT))
  (test (resolve (typecheck (parse `{let {[f : ?
                                             {lambda {[x : ?]}
                                               {lambda {[y : ?]}
                                                 {lambda {[z : ?]}
                                                   {if0 x y z}}}}]}
...
(module+ test
  ...
  (test (unify! a-type-var (varT (box (some a-type-var))) an-expr)
        (values))
  (test (unify! (listofT (varT (box (none)))) (listofT (numT)) an-expr)
        (values))
  ...
  (test/exn (unify! (numT) (arrowT (numT) (boolT)) an-expr)
            "no type")
  (test/exn (unify! (numT) (listofT (numT)) an-expr)
            "no type")
  ...
  (test/exn (unify! a-type-var (arrowT (boolT) a-type-var) an-expr)
            "no type")
  (test/exn (unify! a-type-var (listofT a-type-var) an-expr)
            "no type")
  ...
  (test (occurs? a-type-var (arrowT (numT) a-type-var))
        #t)
  (test (occurs? a-type-var (listofT (numT)))
        #f)
