  ...
  (cond
    ...
    [(s-exp-match? `{letrec {[SYMBOL ANY]} ANY} s)
     (let ([bs (s-exp->list (first
       ...
       (appE (parse mk-rec-fun)
             (lamE (s-exp->symbol (first bs)) (parse (second bs))))))]
...
(module+ test
  ...
  (test(interp (parse (list->s-exp (list (list->s-exp (list plus `2)) `1))) mt-env)
    ...
    (test(interp (parse (list->s-exp (list (list->s-exp (list plus `2)) `4))) mt-env)
       (interp (parse (list->s-exp (list `+ `2 `4))) mt-env))
 ...
 (test (interp (parse (list->s-exp (list (list->s-exp (list times `3)) `7))) mt-env)
       (numV 21))
 (test (interp (parse (list->s-exp (list (list->s-exp (list times `10)) `4))) mt-env)
       (numV 40)))
...
(define plus `{lambda {n}
                {lambda {m}
                  {+ n m}}})
;; times ---------------------------------------
(define times `{lambda {n} {lambda{m}
                 {letrec {[f {lambda {m}
                                      {if0 m
                                           0
                                           {+ {f {+ m -1}} n}}}]}
