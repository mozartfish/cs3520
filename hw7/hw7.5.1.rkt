#lang plait

(define-type Value
  (numV [n : Number])
  (closV [args : (Listof Symbol)]
         [body : Exp]
         [env : Env])
  (contV [k : Cont]))

(define-type Exp
  (numE [n : Number])
  (idE [s : Symbol])
  (plusE [l : Exp] 
         [r : Exp])
  (multE [l : Exp]
         [r : Exp])
  ; negE type
  (negE [e : Exp])

  ; if0E type
  (if0E [tst : Exp]
        [thn : Exp]
        [els : Exp])

  ; avgE type
  (avgE [l : Exp]
        [m : Exp]
        [r : Exp])
  
  (lamE [ns : (Listof Symbol)]
        [body : Exp])
  (appE [fun : Exp]
        [args : (Listof Exp)])
  (let/ccE [n : Symbol]
           [body : Exp]))

(define-type Binding
  (bind [name : Symbol]
        [val : Value]))

(define-type-alias Env (Listof Binding))

(define mt-env empty)
(define extend-env cons)
(define extend-env* append)

(define-type Cont
  (doneK)
  (plusSecondK [r : Exp]
               [e : Env]
               [k : Cont])
  (doPlusK [v : Value]
           [k : Cont])
  
  ;; waiting for value k for doNegK
  (doNegK [k : Cont])

  (multSecondK [r : Exp]
               [e : Env]
               [k : Cont])
  (doMultK [v : Value]
           [k : Cont])
  ;; if0K
  (if0K [thn : Exp]
        [els : Exp]
        [env : Env]
        [k : Cont])
  ;; doIfK - for evaluating thn and els conditions similar
  ;; similar to doAppK
  (doIfK 
   [k : Cont])

  ;; average continue cases (similar to multSecondK and plusSecondK

  ;; l is being evaluated for this case
  ;; evaluate the first expression
  (avgSecondK [m : Exp]
              [r : Exp]
              [e : Env]
              [k : Cont])

  ;; m is being evaluated for this case
  ;; the first expression has already returned a value
  ;; and we now have to evaluate the second expression
  (avgThirdK [v-1 : Value]
             [r : Exp]
             [e : Env]
             [k : Cont])

  ;; r is being evaluated for this case
  ;; expressions l and m have returned values
  ;; and we now have to evaluate the third expression
  (doAvgK [v-l : Value]
          [v-m : Value]
          [k : Cont])
  
  (appArgK [a : (Listof Exp)]
           [env : Env]
           [k : Cont])

  ;; continue for multiple argument expressions
  (continueListK [rst : (Listof Exp)]
                 [v-f : Value]
                 ; list for storing all the values that have been evaluated
                 [vals : (Listof Value)]
                 [e : Env]
                 [k : Cont])
  (doAppK [f : Value]
          ; this list is for storing all the values that have been evaluated
          [vals : (Listof Value)]
          [k : Cont])) 

(module+ test
  (print-only-errors #t))

;; parse ----------------------------------------
(define (parse [s : S-Exp]) : Exp
  (cond
    [(s-exp-match? `NUMBER s) (numE (s-exp->number s))]
    [(s-exp-match? `SYMBOL s) (idE (s-exp->symbol s))]
    [(s-exp-match? `{+ ANY ANY} s)
     (plusE (parse (second (s-exp->list s)))
            (parse (third (s-exp->list s))))]
    ; neg parse
    [(s-exp-match? `{neg ANY} s)
     (negE (parse (second (s-exp->list s))))]
    ; if0 parse
    [(s-exp-match? `{if0 ANY ANY ANY} s)
     (if0E (parse (second (s-exp->list s)))
           (parse (third (s-exp->list s)))
           (parse (fourth (s-exp->list s))))]
    ; avg parse
    [(s-exp-match? `{avg ANY ANY ANY} s)
     (avgE (parse (second (s-exp->list s)))
           (parse (third (s-exp->list s)))
           (parse (fourth (s-exp->list s))))]
    
    [(s-exp-match? `{* ANY ANY} s)
     (multE (parse (second (s-exp->list s)))
            (parse (third (s-exp->list s))))]
    [(s-exp-match? `{let {[SYMBOL ANY]} ANY} s)
     (let ([bs (s-exp->list (first
                             (s-exp->list (second
                                           (s-exp->list s)))))])
       (appE (lamE (list (s-exp->symbol (first bs)))
                   (parse (third (s-exp->list s))))
             (list (parse (second bs)))))]
    [(s-exp-match? `{lambda {SYMBOL ...} ANY} s)
     (lamE (map s-exp->symbol (s-exp->list 
                               (second (s-exp->list s))))
           (parse (third (s-exp->list s))))]
    
    [(s-exp-match? `{let/cc SYMBOL ANY} s)
     (let/ccE (s-exp->symbol (second (s-exp->list s)))
              (parse (third (s-exp->list s))))]

    ;; change function calls to take multiple arguments like in hw2
    [(s-exp-match? `{ANY ANY ...} s)
     (appE (parse (first (s-exp->list s)))
           (map parse (rest (s-exp->list s))))]
    [else (error 'parse "invalid input")]))

(module+ test
  (test (parse `2)
        (numE 2))
  (test (parse `x) ; note: backquote instead of normal quote
        (idE 'x))
  (test (parse `{+ 2 1})
        (plusE (numE 2) (numE 1)))
  (test (parse `{* 3 4})
        (multE (numE 3) (numE 4)))
  (test (parse `{+ {* 3 4} 8})
        (plusE (multE (numE 3) (numE 4))
               (numE 8)))
  (test (parse `{let {[x {+ 1 2}]}
                  y})
        (appE (lamE (list 'x) (idE 'y))
              (list (plusE (numE 1) (numE 2)))))
  (test (parse `{lambda {x} 9})
        (lamE (list 'x) (numE 9)))
  (test (parse `{let/cc k 0})
        (let/ccE 'k (numE 0)))
  (test (parse `{double 9})
        (appE (idE 'double) (list (numE 9))))
  (test/exn (parse `{})
            "invalid input"))

;; interp & continue ----------------------------------------
(define (interp [a : Exp] [env : Env] [k : Cont]) : Value
  (type-case Exp a
    [(numE n) (continue k (numV n))]
    ;; negE
    [(negE e) (interp e env
                      (doNegK k))]
                      
    [(idE s) (continue k (lookup s env))]
    [(plusE l r) (interp l env
                         (plusSecondK r env k))]
    ;; avgE
    [(avgE l m r) (interp l env
                          (avgSecondK m r env k))]
    [(multE l r) (interp l env
                         (multSecondK r env k))]
    [(lamE ns body)
     (continue k (closV ns body env))]

    ;; if0E
    [(if0E tst thn els)
     (interp tst env
             (if0K thn els env k))]
    [(appE fun args) (interp fun env
                             (appArgK args env k))]
    [(let/ccE n body)
     (interp body
             (extend-env (bind n (contV k))
                         env)
             k)]))

(define (continue [k : Cont] [v : Value]) : Value
  (type-case Cont k
    [(doneK) v]
    [(plusSecondK r env next-k)
     (interp r env
             (doPlusK v next-k))]
    [(doPlusK v-l next-k)
     (continue next-k (num+ v-l v))]

    ;; avg continue
    [(avgSecondK m r env next-k)
     (interp m env
             (avgThirdK v r env next-k))]
    [(avgThirdK v-l r env next-k)
     (interp r env
             (doAvgK v-l v next-k))]
    [(doAvgK v-l v-m next-k)
     (continue next-k
               ;; take the sum of the first two values using num+ function
               ;; this returns a value which we add to r value
               ;; which is represented by the current v inthe continuation
               ;;
               (num/ (num+ (num+ v-l v-m)v) (numV 3)))]
    ; neg continue
    [(doNegK next-k)
     (type-case Value v
       [(numV n)
        (continue next-k (numV (* -1 n)))]
       [else  (error 'interp "not a neg")])]
     
    [(multSecondK r env next-k)
     (interp r env
             (doMultK v next-k))]
    [(doMultK v-l next-k)
     (continue next-k (num* v-l v))]

    ; if continue
    [(if0K thn els env next-k)
     (type-case Value v
       [(numV n)
        (if (zero? n)
            (interp thn env (doIfK next-k))
            (interp els env (doIfK next-k)))]
       [else (error 'interp "not a number")])]
    
    [(doIfK next-k)
     (continue next-k v)]
    
    [(appArgK a env next-k)
     (if (empty? a)
         (continue (doAppK v empty next-k) v)
         (interp (first a) env
                 (continueListK (rest a) v empty env k)))]
   

     
 
    
    ;; define the continue list variant
    [(continueListK rst v-f vals env next-k)
     (type-case (Listof Exp) rst
       [empty (continue(doAppK v-f (cons v vals) next-k) v)]
       [(cons fst rst) (interp fst env
                               (continueListK rst v-f (cons v vals) env next-k))])]

    [(doAppK v-f vals next-k)
     (cond
       [(< (length vals) 1) (error 'interp "less than one argument")]
       [else
            (type-case Value v-f
              [(closV ns body c-env)
               (if (not (eq? (length vals) (length ns)))
                   (error 'interp "non-matching args")
                   (interp body
                           (extend-env*
                            (map2 bind ns vals)
                            c-env)
                           next-k))]
              [(contV k-v)
               (if (not (eq? (length vals) 1))
                   (error 'interp "more than one argument")
                   (continue k-v (first vals)))]
              [else (error 'interp "not a function")])])]))

        
    
   

(module+ test
  (test (interp (parse `2) mt-env (doneK))
        (numV 2))
  (test/exn (interp (parse `x) mt-env (doneK))
            "free variable")
  (test (interp (parse `x)
                (extend-env (bind 'x (numV 9)) mt-env)
                (doneK))
        (numV 9))
  (test (interp (parse `{+ 2 1}) mt-env (doneK))
        (numV 3))
  (test (interp (parse `{* 2 1}) mt-env (doneK))
        (numV 2))
  (test (interp (parse `{+ {* 2 3} {+ 5 8}})
                mt-env
                (doneK))
        (numV 19))
  (test (interp (parse `{lambda {x} {+ x x}})
                mt-env
                (doneK))
        (closV (list 'x) (plusE (idE 'x) (idE 'x)) mt-env))
  (test (interp (parse `{let {[x 5]}
                          {+ x x}})
                mt-env
                (doneK))
        (numV 10))
  (test (interp (parse `{let {[x 5]}
                          {let {[x {+ 1 x}]}
                            {+ x x}}})
                mt-env
                (doneK))
        (numV 12))
  (test (interp (parse `{let {[x 5]}
                          {let {[y 6]}
                            x}})
                mt-env
                (doneK))
        (numV 5))
  (test (interp (parse `{{lambda {x} {+ x x}} 8})
                mt-env
                (doneK))
        (numV 16))

  (test (interp (parse `{let/cc k {+ 1 {k 0}}})
                mt-env
                (doneK))
        (numV 0))
  (test (interp (parse `{let {[f {let/cc k k}]}
                          {f {lambda {x} 10}}})
                mt-env
                (doneK))
        (numV 10))

  (test/exn (interp (parse `{1 2}) mt-env (doneK))
            "not a function")
  (test/exn (interp (parse `{+ 1 {lambda {x} x}}) mt-env (doneK))
            "not a number")
  (test/exn (interp (parse `{let {[bad {lambda {x} {+ x y}}]}
                              {let {[y 5]}
                                {bad 2}}})
                    mt-env
                    (doneK))
            "free variable")
  ;; Eager:
  (test/exn (interp (parse `{{lambda {x} 0} {1 2}}) mt-env (doneK))
            "not a function")

  (test (continue (doneK) (numV 5))
        (numV 5))
  (test (continue (plusSecondK (numE 6) mt-env (doneK)) (numV 5))
        (numV 11))
  (test (continue (doPlusK (numV 7) (doneK)) (numV 5))
        (numV 12))
  (test (continue (multSecondK (numE 6) mt-env (doneK)) (numV 5))
        (numV 30))
  (test (continue (doMultK (numV 7) (doneK)) (numV 5))
        (numV 35)))
  (test (continue (appArgK (numE 5) mt-env (doneK)) (closV (list 'x) (idE 'x) mt-env))
        (numV 5))
  (test (continue (doAppK (closV (list 'x) (idE 'x) mt-env) (doneK)) (numV 8))
        (numV 8)))

;; num+ and num* and num/ ----------------------------------------
(define (num-op [op : (Number Number -> Number)] [l : Value] [r : Value]) : Value
  (cond
    [(and (numV? l) (numV? r))
     (numV (op (numV-n l) (numV-n r)))]
    [else
     (error 'interp "not a number")]))
(define (num+ [l : Value] [r : Value]) : Value
  (num-op + l r))
(define (num* [l : Value] [r : Value]) : Value
  (num-op * l r))
(define (num/ [l : Value] [r : Value]) : Value
  (num-op / l r))
  
(define (num-zero? [v : Value]) : Boolean
  (type-case Value v
    [(numV n) (zero? n)]
    [else (error 'interp "not a number")]))

(module+ test
  (test (num+ (numV 1) (numV 2))
        (numV 3))
  (test (num* (numV 2) (numV 3))
        (numV 6))
  (test (num/ (numV 4) (numV 2))
        (numV 2))
  (test/exn (num-zero? (contV (doneK)))
            "not a number")
  (test (num-zero? (numV 0))
        #t)
  (test (num-zero? (numV 1))
        #f))

;; lookup ----------------------------------------
(define (lookup [n : Symbol] [env : Env]) : Value
  (type-case (Listof Binding) env
    [empty (error 'lookup "free variable")]
    [(cons b rst-env) (cond
                        [(symbol=? n (bind-name b))
                         (bind-val b)]
                        [else (lookup n rst-env)])]))

(module+ test
  (test/exn (lookup 'x mt-env)
            "free variable")
  (test (lookup 'x (extend-env (bind 'x (numV 8)) mt-env))
        (numV 8))
  (test (lookup 'x (extend-env
                    (bind 'x (numV 9))
                    (extend-env (bind 'x (numV 8)) mt-env)))
        (numV 9))
  (test (lookup 'y (extend-env
                    (bind 'x (numV 9))
                    (extend-env (bind 'y (numV 8)) mt-env)))
        (numV 8)))

; interp-expr
(define (interp-expr [e : Exp]) : S-Exp
  (type-case Value (interp e mt-env (doneK))
    [(numV n) (number->s-exp n)]
    [(closV arg body env) `function]
    [(contV k) `function]))

;; interp-expr tests
(module+ test
  (test (interp-expr (parse `{lambda {x} x}))
        `function)
  (test (interp-expr (parse `{let/cc x x}))
        `function))

;;not a number and not a neg cases
(module+ test
  (test/exn (interp-expr (parse `{neg {let/cc k k}}))
            "not a neg")
  (test/exn (interp-expr (parse `{if0 {let/cc k k} 2 3}))
            "not a number"))

;; Part 1 Tests
(module+ test
  (test (interp-expr (parse `{neg 2}))
        `-2)
  (test (interp-expr (parse `{avg 0 6 6}))
        `4)
  (test (interp-expr (parse `{let/cc k {neg {k 3}}}))
        `3)
  (test (interp-expr (parse `{let/cc k {avg 0 {k 3} 0}}))
        `3)
  (test (interp-expr (parse `{let/cc k {avg {k 2} {k 3} 0}}))
        `2)
  (test (interp-expr (parse `{if0 1 2 3}))
        `3)
  (test (interp-expr (parse `{if0 0 2 3}))
        `2)
  (test (interp-expr (parse `{let/cc k {if0 {k 9} 2 3}}))
        `9))

(module+ test
  
  (test (interp-expr (parse `{{lambda {x y} {+ y {neg x}}} 10 12}))
        `2)
  (test (interp-expr (parse `{lambda {} 12}))
        `function)
  (test (interp-expr (parse `{lambda {x} {lambda {} x}}))
        `function)
  (test (interp-expr (parse `{{{lambda {x} {lambda {} x}} 13}}))
        `13)

  (test (interp-expr (parse `{let/cc esc {{lambda {x y} x} 1 {esc 3}}}))
        `3)
  (test (interp-expr (parse `{{let/cc esc {{lambda {x y} {lambda {z} {+ z y}}}
                                           1 
                                           {let/cc k {esc k}}}}
                              10}))
        `20)

  (test/exn (interp-expr (parse `{let/cc esc {esc}}))
            ;; error because continuation is given 0 arguments,
            ;; but the specific error message is not specified
            "")
  (test/exn (interp-expr (parse `{let/cc esc {esc 1 2}}))
            ;; error because continuation is given 2 arguments
            ""))
